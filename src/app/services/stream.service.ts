...
export class StreamService {
    ...
    // liveUsersList = [];
    options = {
        appId: 'e083c376ed644d0c8887e9ebdb48f36d', // set your appid here b1a776384fe24b58a43030c834b8f7dd
        channel: '63a9a080c4b96c00167382f4Walgreens Pharma', // Set the channel name.
    ...
    async localUser(token: string, uuid: any, type: string, rtc: IRtc) {
        console.log("callll=====", uuid, type)
        if (type == 'live') {
    ...
    agoraServerEvents(rtc: IRtc, uid1?: any, uid2?: any) {
        console.log("calllllll-", rtc.client)
        // 2 used
        rtc.client.on('user-published', async (user, mediaType) => {
            ...
            if (uid1 != id && this.rtcscreenshare.uid != id && uid2 != id) {
                ...
                if (mediaType === 'video') {
                    ...
                    this.rtcLiveUser.localVideoTrack = this.remoteVideoTrack;
                    setTimeout(() => {
                        this.remoteVideoTrack.play('remote-playerlist' + user.uid);
                    }, 1000);
                }
                if (mediaType === 'audio') {
                    this.remoteAudioTrack = user.audioTrack;
                    this.rtcLiveUser.localAudioTrack = this.remoteAudioTrack;
    ...
    async switchMicrophone(label: string, localTracks: IMicrophoneAudioTrack) {
        ...
        let currentMic: any = mics.find((mic) => mic.label === label);
    ...
    async switchMicrophone2(val, localTracks) {
        ...
        if (val.kind == 'audiooutput') {
            let currentMic: any = mics.find(mic => mic.label === val.label);
            await localTracks.setPlaybackDevice(currentMic.deviceId);
        } else {
            let currentMic2: any = mics.find(mic => mic.label === val.label);
    ...
    switchStream = (client: IAgoraRTCClient, highOrLow: number, streamUid: any) => {
        if (highOrLow === 0) {
            highOrLow = 1

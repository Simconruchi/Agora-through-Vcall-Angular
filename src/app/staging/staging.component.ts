...
export class StagingComponent implements OnInit {
  ...
  setVideo1() {
    if (this.urlId == '1') {
      ...
      console.log(this.stream.rtc, this.stream.rtcLiveUser)
      // this.remoteVideoTrack = user.videoTrack;
      // if(){
      //   this.stream.remoteVideoTrack.play('remote-playerlist' + user.uid);
      // }else{
      //   this.stream.remoteVideoTrack.stop();

      // }
      this.stream.rtcLiveUser.localVideoTrack.setEnabled(false);
    }
  }
  ...
  async setAudio1() {
    if (this.urlId == '1') {
      ...
      console.log(this.stream.rtc, this.stream.rtcLiveUser)
      // if(){
      //   this.stream.remoteAudioTrack.play('remote-playerlist' + user.uid);
      // }else{
      //   this.stream.remoteAudioTrack.stop();

      // }
      this.stream.rtcLiveUser.localAudioTrack.setEnabled(false);
    }
  }
  ...
  async joinCall() {
    ...
    await this.stream.localUser(
      "006e083c376ed644d0c8887e9ebdb48f36dIADeYmz16ZtA1yCRBTWZengEGi/61H1hBEiPfnDQQzPa8c2HeWEAAAAAEACCF2IMCCe0YwEAAQCY47Jj",
      uid,
      'live',
